...
public class CameraController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (mouseMove)
        {
            ...
            if (mousePos.x > Screen.width * 0.97f && mousePos.x < Screen.width)
                moveInput.x = 1;
            else if (mousePos.x < Screen.width * 0.03f && mousePos.x > 0)
                moveInput.x = -1;

            if (mousePos.y > Screen.height * 0.97f && mousePos.y < Screen.height)
                moveInput.z = 1;
            else if (mousePos.y < Screen.height * 0.03f && mousePos.y > 0)
                moveInput.z = -1;
        }
